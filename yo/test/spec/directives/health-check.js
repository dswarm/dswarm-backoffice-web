...
describe('Directive: HealthCheck', function () {
    var element, scope, $rootScope, $compile, $timeout, $httpBackend;
    var elementHtml = '<div health-check endpoint="\'health\'" healthy="\'foo\'"></div>';
    ...
    it('send a ping periodically every second', function() {
        $httpBackend.expectGET('foo/health').respond({foo: {healthy: true}});
    ...
    it('should modify the classses of the container element', function() {
        ...
        $httpBackend.expectGET('foo/health').respond({foo: {healthy: false}});
        $timeout.flush(1000);
        $timeout.flush();
        $httpBackend.flush();
        scope.$digest();
        ...
        $httpBackend.expectGET('foo/health').respond(400, 'gone');
        $timeout.flush(1000);
        $timeout.flush();
        $httpBackend.flush();
        scope.$digest();
    ...
    it('should send no health-check if disabled', function() {
        var classes;
